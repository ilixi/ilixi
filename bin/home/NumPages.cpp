...
D_DEBUG_DOMAIN( ILX_NUMPAGES, "ilixi/Home/pages", "NumPages");
...
NumPages::initPages()
{
    ...
    for (int i = 0; i < numPages; ++i)
    {
        ...
        if (numPages > 1)
        {
            ...
            pageButton->setGeometry(buttonOffset + i * 53, height() - 50, 48,
                                    48);
            pageButton->sigClicked.connect(
                    sigc::bind<unsigned int>(
                            sigc::mem_fun(this, &NumPages::showPage), i));
            addChild(pageButton);

...
BoxLayout::BoxLayout(Widget* parent) :
  WidgetLayout(parent), _alignment(Alignment::Top)
{
}
  ...
  while (it != w.end())
    {
      if (((CacheElement) *it).widget->xConstraint() & ExpandPolicy)
        ...
        {
          if (((CacheElement) *it).widget->minWidth() > 0 && average
              > ((CacheElement) *it).widget->minWidth())
            {
              expandSpace += average - ((CacheElement) *it).widget->minWidth();
